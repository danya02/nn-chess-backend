...
async fn index() -> Json<EngineDescription> {
    ...
    Json(EngineDescription {
         ...
         text_description: "Neural network with hidden layers 1536-4096-2048-512-128 that outputs board evaluations. Iterated over moves, chooses (almost) best evaluation".to_string(),
         variants: variants,
...
async fn get_move(
    ...
    if !idxs.contains(&id) {
        return Err((
            ...
            format!("There is no checkpoint ID {id} for wide model"),
        ));
    }
...
async fn submit_for_analysis(
    ...
    let lru = data
        .entry("wide".to_string()) // If there is no entry with the name of the engine, add a new LRU
        .or_insert_with(|| lru::LruCache::new(NonZeroUsize::new(8).unwrap())); // this engine is large, so store an average number
    ...
    let engine_ref = if let Some(eref) = engine_ref {
        if eref.0.is_finished() {
            ...
            println!("Engine wide/{id} finished abnormally: {why:?}");
            None
        } else {
            Some(eref)
        }
    ...
    let engine_ref = if let Some(eref) = engine_ref {
        ...
        println!("Spawning new engine wide/{id}");
        let (tx, rx) = mpsc::channel(8);
        let handle = tokio::task::spawn_blocking(move || eval_wide::move_predictor(id, rx));
        (handle, tx)
    };
